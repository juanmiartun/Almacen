...
expenseAmountInput.addEventListener("input", function () {
  ...
  exportExcelBtn.addEventListener('click', () => {
    ...
    const totalEfectivo = salesData.filter(s => !s.refunded && s.paymentMethod === 'Efectivo')
      .reduce((a, s) => a + s.total, 0);
    const totalTransferencia = salesData.filter(s => !s.refunded && s.paymentMethod === 'Transferencia')
      .reduce((a, s) => a + s.total, 0);

    const wsData = [
      ['VENTAS'],
      [],
      ['Producto', 'Método de Pago', 'Precio', 'Cantidad', 'Total']
    ];
    ...
    wsData.push([]);
    wsData.push(['TOTAL VENTAS', '', '', '', formatMoney(totalVentas)]);

    wsData.push([]);
    wsData.push(['GASTOS']);
    wsData.push([]);
    wsData.push(['Descripción', 'Valor']);

    expensesData.forEach(g => {
      wsData.push([g.description, formatMoney(g.total)]);
    });

    wsData.push([]);
    wsData.push(['TOTAL GASTOS', formatMoney(totalGastos)]);

    wsData.push([]);
    wsData.push(['RESUMEN']);
    wsData.push([]);
    wsData.push(['Concepto', 'Valor']);
    wsData.push(['Total Ventas', formatMoney(totalVentas)]);
    wsData.push(['Total Reembolsos', formatMoney(totalReembolsos)]);
    wsData.push(['Total Gastos', formatMoney(totalGastos)]);
    wsData.push([]);
    wsData.push(['Resultado Final del Día', formatMoney(resultadoFinal)]);

    wsData.push([]);
    wsData.push(['MÉTODO DE PAGO']);
    wsData.push([]);
    wsData.push(['Método', 'Valor']);
    wsData.push(['Efectivo', formatMoney(totalEfectivo)]);
    wsData.push(['Transferencia', formatMoney(totalTransferencia)]);

    const ws = XLSX.utils.aoa_to_sheet(wsData);

    Object.keys(ws).forEach(key => {
      if (ws[key].v) ws[key].t = 's';
    });

    ws['!cols'] = [{ wch: 25 }, { wch: 20 }, { wch: 15 }, { wch: 10 }, { wch: 15 }];
